...
def test_docstring_defaults_match_code(module):
    # Explicit encoding: Python sources are UTF-8, but read_text() decodes with
    # the locale codec, so cp1252 raises on `ō` (ichimoku) or `“` (cksp) and
    # decodes the other non-ASCII modules to mojibake.
