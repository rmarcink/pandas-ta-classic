...
1. **No silent fallbacks for arguments.** `None` selects the default; any other value the function cannot use raises `ValueError` naming the function, the parameter and the value. Never write `x = int(x) if x and x > 0 else default`, `x if x else default`, `bool(x) if isinstance(x, bool) else default` or an `if x not in choices: x = default` block. Use `_pos_int`, `_pos_float`, `_number`, `_bool_param`, `_str_param`, `get_drift` and `get_offset` from `utils/_core.py`. This binds every library function, not just indicators: `utils/_metrics.py` (`calmar_ratio`, `max_drawdown`) and `utils/_math.py` (`linear_regression`) must raise for an invalid parameter or a mismatched input rather than return a sentinel (`np.nan`, `{}`). *Enforced:* `tests/test_parameter_validation.py`, checklist greps 5–8; utility sentinels by *review*.
...
19. **Never mutate shared or caller-owned state.** Work on copies of shared registries (`Category`), of the caller's objects (`Strategy.ta`), and of any DataFrame/Series a utility function receives — `utils/_time.py` `to_utc` returns `df.copy()`, never reindexes the caller's frame in place. *Enforced:* `TestStrategyDoesNotMutateInputs`; utility non-mutation by *review*.
