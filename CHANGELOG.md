...
### Changed
* **`decreasing(strict=True)` / `increasing(strict=True)` up to ~384× faster (no window recomputation)**: the strict variants ANDed `length - 2` shifted comparisons together in a Python loop (`for x in range(3, length + 1): decreasing = decreasing & (close.shift(x - (drift + 1)) < close_.shift(x - drift))`), i.e. O(n × length) in *pandas operations*. Every term is the same one-bar comparison at a different shift, so they collapse into one windowed conjunction: a new `utils._core._all_shifted()` answers `flags.shift(lo) & … & flags.shift(hi)` from a cumulative count of the False entries, two lookups per bar. The first term keeps its own `drift` gap, exactly as before. Per 20000-bar call: `length=10` `1.42 ms` → `0.31 ms`, `length=50` `3.60` → `0.25 ms`, `length=250` `18.17` → `0.24 ms`, `length=1000` `95.11` → `0.25 ms`; the runtime no longer depends on `length` (exponent `0.97` → `-0.04`). Output is **bit-for-bit identical**, dtype included, across 576 configurations (6 frames × lengths 1–100 × `drift` 1/2/3 × with and without `percent`, both directions). The non-strict default path is untouched.
* **`pwma` up to ~19× faster at long windows (Pascal's triangle by recurrence)**: `pascals_triangle()` built its row with one `math.comb(n, i)` call per entry, and each of those is itself O(n) big-integer work, so the weights cost grew quadratically — at `length=2000` building them took `62 ms` while the weighted-MA convolution they feed took `1.3 ms`. The row now uses the exact integer recurrence `C(n, i+1) = C(n, i) * (n - i) // (i + 1)`. Per 20000-bar `pwma` call: `length=250` `1.19 ms` → `0.47 ms`, `length=1000` `10.36` → `1.61 ms`, `length=2000` `62.59` → `3.34 ms`, which brings its growth in `length` back from an exponent of `1.93` to the `0.95` of the convolution itself. Binomial coefficients are integers and the recurrence divides exactly, so `pascals_triangle` returns **bit-identical** rows (verified for n = 0–1500 across the plain, `weighted` and `inverse` forms, dtype included).
* **`ttm_trend` up to ~116× faster (no window recomputation)**: the TTM Trend built its rolling `hl2` average with `for i in range(1, length): trend_avg = trend_avg + hl2(high.shift(i), low.shift(i))` — a Python loop over `length` full-series shifts, `hl2()` calls and Series additions, i.e. O(n × length) in *pandas operations*. That sum divided by `length` is the rolling mean of `hl2`, so it is now one `sma()` call and the runtime no longer depends on `length` at all (a fit of runtime against `length` drops from an exponent of `0.97` to `~0`). Per 10000-bar call: `length=6` `0.64 ms` → `0.34 ms`, `length=30` `2.99` → `0.35 ms`, `length=320` `34.18` → `0.37 ms`. Output is **bit-for-bit identical** across 147 configurations (7 frames × lengths 2–200 × offsets `None`/`2`/`-1`), including the cases where a differing summation order could flip the `close > trend_avg` comparison: an exactly tied flat series, a coarse price grid, NaN in `close` and in `high`, and a `1e9`-scale series. No numba involved.
* **`performance` extra now requires `numba>=0.63.0`** (was `>=0.58.0`): 0.63.0 is the first numba release with cp314 wheels, and the CI matrix runs up to Python 3.14; everything below 0.60 additionally caps numpy at `<2.0`, which contradicts the project's own `numpy>=2.0` floor. Installs that resolved to one of those versions got the silent plain-Python fallback (or, since the previous release, the `RuntimeWarning` about a broken numba install) instead of a working JIT.
* **`uo` ~2.4× faster (pure numpy)**: the Ultimate Oscillator built a three-column DataFrame of `high`, `low` and the shifted close only to reduce two pairs of it with `.loc[:, [...]].max(axis=1)` / `.min(axis=1)` — an intermediate frame plus two row-wise reductions per call, together ~60% of the indicator's profile. Both pairs are now `np.fmax` / `np.fmin`, which skip NaN exactly like `DataFrame.max(axis=1)`'s default `skipna=True` and still align the two Series, so the result is **bit-for-bit identical**, NaN mask included (verified on 200- and 5000-bar frames). Per 5000-bar call `1.98 ms` → `0.84 ms`. No numba involved — the win applies to every install.
...
* **`vp` ~2.3× faster (pure numpy)**: the Volume Profile sliced its working frame into `width` price buckets with `vp.iloc[idx]` and reduced each one with five pandas aggregations — 10 DataFrame copies and 50 reductions at the default `width=10`. The three columns are now split as numpy views (`np.array_split` over the same contiguous ranges) and reduced with `np.nanmin` / `np.nanmean` / `np.nanmax` / `np.nansum`, which match what the pandas reductions did with their default `skipna=True`; the two volume columns also use `.where` instead of a boolean subset, so the products no longer realign. Per 5000-bar call `2.74 ms` → `1.20 ms` (`width=10`), **bit-for-bit identical** at 5000/233/50 bars and `width` 10/7/3. No numba involved.
* **`df.ta` accessor ~40× cheaper to construct**: pandas builds a fresh `AnalysisIndicators` on every `df.ta` attribute access, and `__init__` stamped `_last_run` by calling `get_time()` — a `Timestamp.now()` plus `day_name()`, `month_name()`, `timetuple()` and `localtime()` formatting pass, `8.3 µs` per access. That is more than some indicators' entire calculation (`df.ta.sma()` on 5000 bars is `~50 µs`). The timestamp is now computed on first read of the `last_run` property instead, so a fresh accessor still reports the current time and a run still stamps its own; `df.ta` drops `8.3 µs` → `0.2 µs`.
...
* **`cdl_pattern(name="all")` ~1.3× faster (no redundant work)**: every native `cdl_*` pattern re-derived the same nine OHLC helper arrays (body high/low, real body, upper/lower shadow, ranges, colour) from the same four `to_numpy()` conversions, so a single `cdl_pattern` call built 60 identical `CandleArrays` — the largest entry in its profile once the detection loops were JIT-compiled. `cdl_pattern` now builds them once and passes them to `run_pattern`, which takes them as an explicit `candle_arrays` parameter; it reaches the patterns' detection kernels directly rather than through the public `cdl_*` wrappers, so nothing new travels through those wrappers' `**kwargs`. A pattern invoked on its own still builds its own arrays, and a `cdl_pattern` run that reaches no native pattern builds none. Per 5000-bar call `9.72 ms` → `7.41 ms`. Output is **bit-for-bit identical** for all 60 patterns, verified across the full 456-column indicator sweep as well as the default call, `scalar=1`, `offset=2`, and the standalone `cdl_hammer` / `cdl_doji` / `cdl_inside`. No numba involved.
