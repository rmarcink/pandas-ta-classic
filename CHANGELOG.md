...
### Removed
* **Six unreferenced documentation images** (`docs/images/SPY_ADX.png`, `SPY_ATR.png`, `SPY_CumulativePercentReturn.png`, `SPY_MACD.png`, `SPY_OBV.png`, `SPY_ZScore.png`): no page, README or notebook referenced them; the only hits repo-wide were the generated `SOURCES.txt` / `scm_file_list.json` manifests that list them because they were tracked. `TA_Chart.png` and `logo.png` are still used by the README and stay.
...
### Fixed
* **Indicator compatibility matrix was published twice**: `docs/indicators.rst` ended in `.. include:: indicator_support_matrix.rst` while Sphinx also built that file as its own document, so the 225-row table appeared both inline in `indicators.html` and on a standalone `indicator_support_matrix.html` that no toctree linked to — duplicated in the site search, and the standalone copy reachable only by guessing the URL. The include is now a `.. seealso::` cross-reference and the matrix is a real entry in the *User Guide* toctree. `indicators.html` drops from 74.7 kB to 43.3 kB (225 table rows → 0), the matrix keeps all 225, and the page is linked from the sidebar. The included file also carried its own `====` title, which had been injecting a second top-level heading into `indicators.html`.
* **`docs/testing.rst` documented a test setup the project no longer has**: it named `python -m unittest` as the primary runner "matching CI" (CI runs `python -m pytest` exclusively), claimed the oracle suite "requires `ta-lib` **and tulipy** installed" (tulipy was dropped from the `[oracle]` extra — `test_oracle_tulipy.py` compares against the committed `tests/fixtures/tulipy_oracle.json` and imports nothing; verified: 59 passed with no tulipy on the interpreter, which matters because tulipy ships no wheels for CPython 3.12+), pointed at a non-existent `ta.custom.bind()` path (`bind` lives in `pandas_ta_classic.custom`), and said `test_strategy.py` "runs separately from the main suite" when `pytest tests/` collects it. Three test modules had no entry at all: `test_accessor_conformance.py`, `test_lazy_core.py` and `test_njit_fallback.py` — the latter two now covered by a new *Infrastructure Tests* section. The *Fixture Files* section also names `generate_tulipy_oracle.py` and its CPython <3.12 constraint, which was only documented in `installation.rst`.
* **Three dead documentation links**: `docs/quickstart.md`, `docs/tutorials.md` and `CONTRIBUTING.md` linked to GitHub Discussions, which is not enabled on the repository (404). Those links are now Issues links, or dropped. `docs/installation.rst` linked `ta-lib-python#dependencies`; GitHub renders that heading's id with a trailing hyphen, so the fragment never resolved for a reader either — it now links the README without a fragment. `docs/testing.rst` followed a redirect to the Hypothesis docs and now points at the final URL. Found with `sphinx-build -b linkcheck`, which additionally reports `CONTRIBUTING.md#version-management` as broken; that one is a false positive, since GitHub prefixes every heading id it renders from Markdown with `user-content-` and resolves the plain fragment client-side.
* **Sphinx `conf.py` header was corrupted**: a stray paste had spliced the epub option block into the module docstring comment, truncating the boilerplate mid-sentence (`# full list# The unique identifier of the text...`) and duplicating `epub_identifier` / `epub_uid`, which already appear in the epub section further down. The header now reads as the intended Sphinx boilerplate; the duplicated lines are gone. No build behaviour change — the lines were all comments.
