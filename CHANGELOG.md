...
### Changed
* **`ichimoku(append_span=True)` with the tuple return now raises `ValueError`**: it was silently dropped. The tuple already hands the projected span back as its second element, so there is nothing to append it to — the combination is a contradiction, not a no-op, and swallowing it could hand back a frame without the rows the caller asked for. The guard fires for an explicit `as_dataframe=False` and for the deprecated default alike, and runs before the `DeprecationWarning` the default would otherwise emit. Nothing in the wild breaks: at 0.6.52 the underlying `ichimoku()` function had neither `append_span` nor `as_dataframe` (`append_span` was accessor-only and the accessor did the concat itself), so this parameter combination has never shipped. The guard disappears together with the tuple branch once the single-DataFrame return becomes the default.
* **`ichimoku()` deprecation message no longer promises silence it cannot keep**: it advised `as_dataframe=False` "to silence this warning while keeping the tuple", but under the planned two-step migration that same call starts warning once the default flips to a single DataFrame. It now says the silence holds for the current release line only, and names what changes afterwards.
...
* **`ichimoku()` tuple return**: returning a `(visible, span)` tuple is deprecated and now emits a `DeprecationWarning`. Pass `as_dataframe=True` to opt in to the single-DataFrame return (add `append_span=True` for the projected span rows), or `as_dataframe=False` to keep the tuple and silence the warning **for the current release line** — once the default flips, asking for the tuple is what warns. The accessor `df.ta.ichimoku()` already returns a DataFrame and is unaffected: it accepts a redundant `as_dataframe=True` and rejects `as_dataframe=False` with a `TypeError`.
...
### Fixed
* **`df.ta.ichimoku(as_dataframe=True)` raised `TypeError`**: the accessor fixes `as_dataframe=True` when calling the underlying function and forwarded `**kwargs` unchanged, so the same keyword arriving from the caller collided — `TypeError: ichimoku() got multiple values for keyword argument 'as_dataframe'`. The deprecation warning on `ichimoku()` explicitly tells callers to pass `as_dataframe=True`, so following that advice on the accessor crashed. The redundant `True` (and an explicit `None`) is now accepted as a no-op; `as_dataframe=False` raises a `TypeError` that names the alternative, because the accessor appends its result to the frame and cannot hand back a tuple. Covered by three new tests in `tests/test_ext_indicator_overlap_ext.py`.
